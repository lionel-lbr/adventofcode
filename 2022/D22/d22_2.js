/*
Advent Of Code 2022
Day 22: xxx part 2

https://adventofcode.com/2022/day/22
*/

const fs = require('fs');
const path = require('path');

const YEAR = '2022';
const DAY = '22';

function readInput(filename) {
  const readRawIntput = () => {
    const data = fs.readFileSync(path.join(`${YEAR}`, `D${DAY}`, filename));
    const lines = data
      .toString()
      .split(`\n`)
      .filter((l) => l);
    return lines;
  };

  const parseALine = (line) => {};

  try {
    const lines = readRawIntput();
    // return lines.map((l) => l.split(''));
    return lines;
  } catch (err) {
    console.error(err);
  }
}
const SAMPLE_PATH = '10R5L5R10L4R5L5';
const INPUT_PATH =
  '';

const PATH = INPUT_PATH.split('').reduce((list, c) => {
  if (c === 'L' || c === 'R') {
    list.push(c);
    return list;
  }
  // convert 2 digits to 1 integer.
  const last = list.length - 1;
  if (last === -1 || list[last] === 'L' || list[last] === 'R') list.push(Number(c));
  else list[last] = list[last] * 10 + Number(c);
  return list;
}, []);

function Face(map, width, height) {
  const map = map;
  const width = width;
  const height = height;

  this.xInc = 1;
  this.yInc = 1;
  this.connection = {
    top: null,
    bottom: null,
    right: null,
    left: null,
  };

  this.toRight = (x) => {
    x = x + this.xInc;
    if (this.xInc > 0 && x >= width) return [0, this.connection.right];
    if (this.xInc < 0 && x < 0) return [this.width - 1, this.connection.right];
    return [x, this];
  };
  this.toLeft = (x) => {
    x = x - this.xInc;
    if (this.xInc > 0 && x < 0) return [0, this.connection.left];
    if (this.xInc < 0 && x >= width) return [this.width - 1, this.connection.left];
    return [x, this];
  };

  this.toDown = (y) => {
    y = y + this.yInc;
    if (this.yInc > 0 && y >= height) return [0, this.connection.bottom];
    if (this.yInc < 0 && y < 0) return [this.height - 1, this.connection.bottom];
    return [y, this];
  };

  this.toUp = (y) => {
    y = y - this.yInc;
    if (this.yInc > 0 && y < 0) return [this.height - 1, this.connection.top];
    if (this.yInc < 0 && y >= height) return [this.height - 1, this.connection.top];
    return [y, this];
  };
}

function Front(input, x, y) {
  Object.assign(this, new Face(input, x, y));
}
function part2(input, width, height) {
  // build the cube
  const facesCoordinates = {
    front: { x: 50, y: 0 },
    right: { x: 100, y: 0 },
    bottom: { x: 50, y: 50 },
    back: { x: 50, y: 100 },
    left: { x: 0, y: 100 },
    top: { x: 0, y: 150 },
  };

  const getFace = ({ x, y }) => {
    const face = [];
    for (let i = 0; i < height; i++) {
      const row = input[y + i].slice(x, x + width + 1);
      face.push(row);
    }

    return face;
  };
  const faces = {
    front: new Face(getFace(facesCoordinates.front), width, height),
    right: new Face(getFace(facesCoordinates.right), width, height),
    bottom: new Face(getFace(facesCoordinates.bottom), width, height),
    back: new Face(getFace(facesCoordinates.back), width, height),
    left: new Face(getFace(facesCoordinates.left), width, height),
    top: new Face(getFace(facesCoordinates.top), width, height),
  };

  // link face as a cube

  front.connection.top = faces.top;
  front.connection.bottom = faces.bottom;
  front.connection.right = faces.right;
  front.connection.left = faces.left;

  right.connection.top = faces.top;
  right.connection.bottom = faces.bottom;
  right.connection.right = faces.back;
  right.connection.left = faces.front;

  back.connection.top = faces.top;
  back.connection.bottom = faces.bottom;
  back.connection.right = faces.left;
  back.connection.left = faces.right;

  left.connection.top = faces.top;
  left.connection.bottom = faces.bottom;
  left.connection.right = faces.front;
  left.connection.left = faces.back;

  top.connection.top = faces.back;
  top.connection.bottom = faces.front;
  top.connection.right = faces.right;
  top.connection.left = faces.left;

  bottom.connection.top = faces.back;
  bottom.connection.bottom = faces.front;
  bottom.connection.right = faces.left;
  bottom.connection.left = faces.right;

  let direction = 'RIGHT';
  let posX = 0;
  let posY = 0;
  let currentFace = faces['front'];

  const getNextPosition = (posX, posY, direction, face) => {
    let x = posX;
    let y = posY;
    while (true) {
      switch (direction) {
        case 'RIGHT':
          [x, face] = face.move.toRight(x);
          break;
        case 'DOWN':
          [y, face] = face.move.toDown(y);
          break;
        case 'LEFT':
          [x, face] = face.move.toLeft(x);
          break;
        case 'UP':
          [y, face] = face.move.toUp(y);
          break;
      }
      const c = face.getCell(x, y);
      if (c === ' ') continue;
      return { x, y, c, face };
    }
  };

  let stepCount = 0;
  while (true) {
    if (stepCount === 0) {
      if (pathIndex === PATH.length - 1)
        return 1000 * (posY + 1) + 4 * (posX + 1) + ['RIGHT', 'DOWN', 'LEFT', 'UP'].findIndex((f) => f === direction);

      // move up in PATH and change direction
      const { d, sc } = setNextDirection(direction, currentFace);
      direction = d;
      stepCount = sc;
    }

    const { x, y, c, face } = getNextPosition(posX, posY, direction, currentFace);
    currentFace = face;
    if (c === '.') {
      posX = x;
      posY = y;
      stepCount -= 1;
      continue;
    }
    if (c === '#') {
      stepCount = 0;
      continue;
    }
  }

  return 0;
}

//const inputPart2 = readInput(`d${DAY}-sample.txt`);
//console.log(`Part 2: ${part2(inputPart2, 4 ,4)}`);
const inputPart2 = readInput(`d${DAY}-input.txt`);
console.log(`Part 2: ${part2(inputPart2, 50, 50)}`);
