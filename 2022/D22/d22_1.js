...
function part1(input) {
  ...
  const getNextPosition = (posX, posY, direction) => {
    let x = posX;
    let y = posY;
    while (true) {
      let width = input[y].length;
      switch (direction) {
        case 'RIGHT':
          x = (x + 1) % width;
          break;
  ...
  const setNextDirection = (direction) => {
    if (pathIndex === -1) {
      pathIndex += 1;
      return { d: 'RIGHT', sc: PATH[pathIndex] };
    }
    pathIndex += 1;
    const d = PATH[pathIndex];
    switch (direction) {
      case 'RIGHT':
    ...
    pathIndex += 1;
    return { d: direction, sc: PATH[pathIndex] };
  };
  ...
  while (true) {
    if (stepCount === 0) {
      ...
      const { d, sc } = setNextDirection(direction);
      direction = d;
      stepCount = sc;
    }

    const { x, y, c } = getNextPosition(posX, posY, direction);
    if (c === '.') {
      posX = x;
      posY = y;
      stepCount -= 1;
